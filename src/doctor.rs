...
fn report(
    ...
    for slug in &slugs {
        ...
        let problems = project::priming_problems(&project, &prefix, env, config_dir);
        if problems.is_empty() {
            ...
            if fix {
                match project::write_priming(&project, &prefix, env, config_dir) {
                    Ok(()) => {
                        left = project::priming_problems(&project, &prefix, env, config_dir);
        ...
        Ok(config) => {
            ...
            for slug in &slugs {
                ...
                for role in [crate::profiles::Role::Thread, crate::profiles::Role::Coordinator] {
                    match crate::profiles::resolve(&config, &safety, &settings, role, None, slug) {
                        Err(error) => check(&mut out, Some(false), &format!("profiles {slug}"), format!("{} {error:#}", role.default_key())),
                        // `omp-<name>` resolves for any valid name, so a typo
                        // shows only when herdr refuses the launch; a remote
                        // thread may still run it on its machine.
                        Ok(profile) if profile.agent() == "omp" && !profile.entry.omp_profile.is_empty() && !crate::omp::agent_dir(env, &profile.entry.omp_profile).is_dir() => check(
                            &mut out,
                            None,
                            &format!("profiles {slug}"),
                            format!("{} {} runs OMP profile `{}`, which has no agent directory on this machine ({}); local launches fail until it exists", role.default_key(), profile.name, profile.entry.omp_profile, crate::omp::agent_dir(env, &profile.entry.omp_profile).display()),
                        ),
                        Ok(_) => {}
                    }
                }
...
mod tests {
    ...
    fn a_broken_profile_config_is_named_with_its_own_hint_not_the_left_alone_one() {
        ...
        assert!(line.ends_with(&format!("; fix {}, or run `set demo coordinator_profile <profile>` to use another profile", config.display())), "{line}");
    ...
    fn colliding_agent_names_fail_the_report() {
        ...
        for suffix in ["a", "b"] {
            ...
            project::write_priming(&project, &crate::coordinator::current_prefix(&root).unwrap(), &env, &home.path().join("cfg")).unwrap();
        }
    ...
    #[test]
    fn a_default_omp_profile_missing_here_is_a_warning() {
        let home = tempfile::tempdir().unwrap();
        let env = Env::for_test(home.path(), &[]);
        let runner = runner_with_herdr("herdr 0.9.1\n");
        let root = home.path().join("root");
        ...
        std::fs::write(project.project_md(), crate::settings::set_in(&text, "thread_profile", "omp-typo").unwrap()).unwrap();
        let (text, _) = report(&env, &root, &cfg, &SessionFlags::default(), &runner, false, None);
        assert!(text.contains("[warn] profiles demo: thread_profile omp-typo runs OMP profile `typo`, which has no agent directory on this machine ("), "{text}");
        std::fs::create_dir_all(crate::omp::agent_dir(&env, "typo")).unwrap();
        let (text, _) = report(&env, &root, &cfg, &SessionFlags::default(), &runner, false, None);
        assert!(!text.contains("[warn] profiles demo"), "{text}");
    }
}
