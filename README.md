...
- Each project folder gets `.omp/config.yml`: an OMP coordinator can never run `routine approve`, `configure`, `unconfigure`, `safety yolo`, `safety set` or `profile add/edit/remove/allow/default`, and `thread resolve`, `sweep`, `archive`, `delete` and every `eval` or `debug` call wait for you to confirm, also under `approvalMode: yolo`. The global `bash.patterns` of the OMP profile the coordinator runs under are repeated there, with their `deny` rules ahead of the confirmations, so they still apply.
- OMP profiles are [agent profiles](docs/operations.md#agent-profiles): `omp` is OMP's default profile, every `~/.omp/profiles/<name>` is the built-in `omp-<name>`, and your own `[profiles.<x>]` with `agent = "omp"` picks one with `omp_profile = "<name>"`. The profile's `model` and `effort` become OMP's `--model` and `--thinking`. A named OMP profile must be in Herdr's `[session.omp_launchers]` and needs a Herdr build with `agent start --profile`, running as the server (restart or hand off the server after you install it).
